...
use clap::Subcommand;
use lopdf::{
    self, Bookmark, Document, Object, ObjectId, Stream,
...
fn generate_basic_pdf(doc_name: &str, pages: u8) -> Result<()> {
    ...
    let pages_ids: Vec<_> = (1..=pages)
        .map(|page_number| append_random_page_to_doc(page_number, doc_name, &pages_id, &mut doc))
        .collect::<Result<_>>()?;
